...
요청에 `source_problems`(학생이 방금 틀린 원본 문제의 실제 내용)가 있으면:
6. **check_type을 원본과 똑같이 맞추세요.** function_call이면 function_call로,
   stdout_match면 stdout_match로. 형식이 바뀌면 학생이 방금 익힌 입출력 방식을
   처음부터 다시 파악해야 해서 복습 효과가 떨어집니다.
7. function_call이면 function_name은 원본과 **다른** 이름으로 짓되, 원본과 같은
   종류의 입력을 받게 하세요 (예: 원본이 리스트를 받으면 새 문제도 리스트).
8. 난이도는 원본과 비슷하게 유지하세요. 같은 지점에서 또 막히도록 어렵게 만들면
   복습이 아니라 좌절입니다.
9. description은 원본과 같은 마크다운 구조(`## 문제`, `## 입력`, `## 출력`)를
   따르세요 — 학생이 읽는 화면이 갑자기 달라 보이면 안 됩니다.
"""
