...
type StudentStatus = '순조로움' | '관찰 필요' | '도움 필요'
...
const STUDENTS: Student[] = [
  ...
  { id: 'STU-006', name: '윤도현', email: 'dohyun@univ.ac.kr', progress: 65, solved: 17, attempts: 25, lastActive: '2시간 전', status: '관찰 필요', weakConcept: '중첩 반복' },
]
...
export default function EducatorPage() {
  ...
  const needsHelp = STUDENTS.filter((student) => student.status === '도움 필요').length
      ...
      <div className="educator-container">
        <header className="educator-heading">
          <div>
            ...
            <h1>Python 기초 01</h1>
            <p>2026 여름학기 · 수강생 28명 · 담당 교수 김튜토리</p>
          </div>
          <button className="educator-primary"><Send size={15} /> 공지 보내기</button>
        </header>

        <section className="educator-metrics" aria-label="수업 요약">
          <Metric icon={<UsersRound />} label="전체 수강생" value="28명" note="지난주 대비 +2" />
          <Metric icon={<TrendingUp />} label="평균 진도율" value="64%" note="이번 주 +8%" />
          <Metric icon={<CheckCircle2 />} label="문제 완료율" value="71%" note="전체 728회 풀이" />
          <Metric icon={<AlertTriangle />} label="도움 필요" value={`${needsHelp}명`} note="최근 3일 기준" alert />
        </section>
        ...
        <div className="educator-dashboard-grid">
          <section className="educator-panel educator-attention">
            ...
            <div className="attention-list">
              {STUDENTS.filter((student) => student.status !== '순조로움').slice(0, 3).map((student) => (
            ...
            <div className="assignment-list">
              {ASSIGNMENTS.map((assignment) => (
                <div key={assignment.title}>
                  ...
                  <div className="assignment-progress"><span><i style={{ width: `${(assignment.completed / assignment.total) * 100}%` }} /></span><small>{assignment.completed}/{assignment.total}명</small></div>
        ...
        <section className="educator-panel student-management">
          <div className="student-management-heading">
            <PanelTitle icon={<BarChart3 />} title="수강생 학습 현황" caption="진도와 최근 학습 활동을 한눈에 살펴보세요" />
