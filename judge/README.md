...
**`harness/` 를 고치거나 하네스를 추가했으면 반드시 다시 빌드한다.**

컨테이너는 레포의 파일이 아니라 **이미지에 굳어 있는 사본**을 실행한다. 그래서
이미지가 낡으면 두 가지 방식으로 잘못된다.

- **파일이 추가됐는데 이미지에 없으면**: 컨테이너가 "No such file" 을 뱉고 채점이
  전부 실패한다. 결과는 `INTERNAL_ERROR` — 학생 코드 문제가 아니라 채점기 문제라는 신호다.
- **파일 내용이 바뀌었는데 이미지가 옛것이면**: 아무 에러 없이 **옛 하네스가 그대로
  돌아간다.** 이게 더 위험하다. judge 를 망가뜨려도 테스트가 초록색으로 나온다
  (실측: `run_stdout_match.py` 의 채점 판정을 `passed = False` 로 바꾸고 재빌드하지
  않았더니 `test_stdout_match.py` 3개가 전부 통과했다).

그래서 **`pytest` 가 세션 시작 시 이미지 안의 하네스를 해시로 대조한다**
(`conftest.py`). 다르면 어느 파일이 어긋났는지와 함께 빌드 명령을 알려주고 멈춘다.
Docker 자체가 없으면 skip 한다.

> 단 이 가드는 테스트 경로에만 있다. 데모 직전에 하네스를 고쳤다면 **테스트를 한 번
> 돌려서** 이미지가 최신인지 확인하는 것이 가장 싼 점검이다.
